...
#define LEDA_ROOT_INCL_ID 520531
...
void          ref_plus()  { lock(); count++; unlock(); }
unsigned long ref_minus() { lock(); count--; unlock(); return count; }
...
static inline void normalize_rep (bigfloat_rep* z) {
  ...
  long exp = z->exponent;
  if (z->exp_ptr!=NULL || exp > MAXEXP || exp < MINEXP ) 
