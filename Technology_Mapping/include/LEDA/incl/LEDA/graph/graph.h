...
#define LEDA_ROOT_INCL_ID 520296
...
/*{\Mop        restores $e$ by appending it to $adj\_edges(source(e))$
   ...
   void restore_edges(const list<edge>& el) { edge e; forall(e, el) restore_edge(e); }
...
/*{\Mtext       
\bigskip
{\bf e) Operations for undirected graph} }*/
...
$\{$ ``the edges of $adj\_edges(w)$ and $in\_edges(w)$ are successively 
       assigned to $e$" $\}$ 
}*/
...
/*{\Mop        adds a new node $v=\<x\>$ to |\Mvar| and returns it. $v$ is 
               inserted in front of (|dir=leda::before|) or behind (|dir=leda::behind|)
               node $u$ into the list of all nodes. }*/
