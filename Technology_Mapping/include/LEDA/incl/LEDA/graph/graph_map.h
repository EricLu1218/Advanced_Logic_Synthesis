...
#define LEDA_ROOT_INCL_ID 520202
...
{ error_handler(1,"Compile with -DLEDA_GMAP_IO for graph_map I/O."); }
...
{ 
#if ! defined(LEDA_CHECKING_OFF)
if (!g->member(v)) error_handler(999,"node_map: illegal node");
#endif
  if (g_index > -1) 
    return v->data(g_index); 
  else
...
{
#if ! defined(LEDA_CHECKING_OFF)
if (!g->member(e)) error_handler(999,"edge_map: illegal edge");
#endif
  if (g_index > -1) 
    return e->data(g_index); 
  else
...
{
#if ! defined(LEDA_CHECKING_OFF)
if (!g->member(f)) error_handler(999,"face_map: illegal face");
#endif
  if (g_index > -1) 
    return f->data(g_index); 
  else
...
friend ostream& operator<<(ostream& ostr, const graph_map<graph_t>&) { return ostr; }
friend istream& operator>>(istream& istr, graph_map<graph_t>&) { return istr; }
#endif

};


#if LEDA_ROOT_INCL_ID == 520202
