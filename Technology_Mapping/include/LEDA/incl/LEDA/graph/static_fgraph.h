...
{ 
  ...
  { st_graph::finish_construction();
    ...
    { V_first_in[st_graph::index(v)] = (edge)v->FirstInEdge;
      ...
      edge e;
      forall_in_edges(e,v)
      { E_pred_in[st_graph::index(e)] = (unsigned long)p;
        assert(((unsigned long)e & 0x00000001) == 0);
        p = e;
       }
      if (v->first_in_edge()) 
         E_pred_in[st_graph::index(v->first_in_edge())] = (unsigned long)p;
     }
   }
    ...
    { edge e_stop = v->first_out_edge();
      ...
      { node u = opposite(e,v);
        ...
        { unsigned long last = E_pred_in[st_graph::index(f)];
          E_pred_in[st_graph::index(f)] = (unsigned long)e;
          E_pred_in[st_graph::index(e)] = last;
         }
        else
          E_pred_in[st_graph::index(e)] = (unsigned long)e;
       }
     }
    forall_nodes(v,(*this)) 
         V_first_in[st_graph::index(v)] = (edge)v->FirstInEdge;
