...
// $Revision: 1.9 $  $Date: 2007/02/25 00:48:48 $
...
{ 
  ...
  { return (v+1)->first_out_edge() - v->first_out_edge(); }
};
      ...
      : public st_edge_struct<directed_graph> 
{
  ...
  { 
    ...
    for (node v = G.first_node(); v != G.stop_node(); v = G.next_node(v))
    {
      ...
      edge e0 = v->first_in_edge(), e1 = e0, e2 = e0;
      while (e0->NextInEdge != 0)
      {
        e2 = (edge) e0->NextInEdge;
        e0->NextInEdge = (edge) e2->NextInEdge;
        e2->NextInEdge = e1;
        e1 = e2;
      }
...
class st_edge_struct<opposite_graph> 
{
  ...
  unsigned short SrcXOrTar;
  unsigned short usr_data;
#else
  unsigned long SrcXOrTar;
#endif
...
#if defined(NODE_SIZE_LOG)
    SrcXOrTar = (unsigned short)G.index(v) ^ (unsigned short)G.index(w);
#else
    SrcXOrTar = (unsigned long)v ^ (unsigned long)w; 
#endif
  ...
  { return (node)((unsigned long)v ^ SrcXOrTar); }
...
*/
 ...
 ~static_graph() { clear(); }
...
#if defined (NODE_SIZE_LOG)
    assert(sizeof(node_type) == (1 << NODE_SIZE_LOG));
    assert(n+2 < (1<<16));
#endif

    if (V == 0) V = (char*)std_memory.allocate_vector((n+2)*sizeof(node_type));
    v_first = node(V+sizeof(node_type) - (unsigned long)V % sizeof(node_type));
    v_last  = v_first - 1;
    v_end   = v_first + n;

    if (E == 0) E = (char*)std_memory.allocate_vector((m+2)*sizeof(edge_type));
    e_first = edge(E+sizeof(edge_type) - (unsigned long)E % sizeof(edge_type));
    e_last  = e_first - 1;
    e_end   = e_first + m;
  ...
  int  index(edge e) const { return e - e_first; }

  int  max_node_index()  const { return v_last - v_first; }
  int  max_edge_index()  const { return e_last - e_first; }
 
  int  number_of_nodes() const { return v_end - v_first; }
  int  number_of_edges() const { return e_end - e_first; }
