...
// $Revision: 1.5 $  $Date: 2007/02/25 00:48:37 $
...
for(void* loop_stop = ((v = (G).first_node()),(G).stop_node());\
    v != loop_stop; v = (G).next_node(v))

#undef  forall_edges
#define forall_edges(e,G)\
LEDA_FORALL_PREAMBLE \
for(void* loop_stop = ((e = (G).first_edge()),(G).stop_edge());\
    e != loop_stop; e = (G).next_edge(e))
...
LEDA_FORALL_PREAMBLE \
for(void* loop_stop = ((e = v->first_in_edge()), v->stop_in_edge());\
    e != loop_stop; e = e->next_in_edge())

#undef  forall_adj_edges
#define forall_adj_edges(e,v) forall_out_edges(e,v)

// macros have been defined differently from std graph (cf. graph.h)
// so templates specialized for the std graph cease to work
#define LEDA_NO_TEMPL_FCTS_FOR_STD_GRAPH
...
struct opposite_graph
{
  ...
  { 
    ...
    typename edge_t::graph::node target(edge_t* e) const 
    { typedef typename edge_t::graph::node node;
      error_handler(1,"opposite_graph::target(e) not available.");
      return 0; 
    }
    ...
    typename edge_t::graph::node source(edge_t* e) const 
    { typedef typename edge_t::graph::node node;
      error_handler(1,"opposite_graph::source(e) not available.");
      return 0; 
    }
  };
};
