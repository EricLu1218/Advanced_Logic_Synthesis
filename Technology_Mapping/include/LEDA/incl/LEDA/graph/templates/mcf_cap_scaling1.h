...
// $Revision: 1.5 $  $Date: 2007/02/28 07:44:14 $

#if !defined(LEDA_ROOT_INCL_ID)
#define LEDA_ROOT_INCL_ID 520219
...
*/
  ...
  int f = MAX_FLOW(A,s,t,A.edge_data(),A_flow);

/*
if (write_log) cout << "fmax = " << f << endl;
*/
...
*/
  ...
  { 
    ...
    while (m_count-- > 0)
    { if (!DIJKSTRA_IN_RESIDUAL1(G,delta,cost,cap,flow,excess,pot,dist,
                                                    mark,PQ,V,++count)) break;
       MAXFLOW_IN_ADMISSIBLE(G,delta,cost,cap,pot,excess,pred,flow);
     }
  ...
  { if (excess[v]  != 0) 
    { feasible = false;
      //cout << string("v = %d  excess = %d",G.index(v),excess[v]) << endl;
     }
   }
