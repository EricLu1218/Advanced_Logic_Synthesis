...
#define LEDA_ROOT_INCL_ID 520231
...
#define RCOST(e,u,v) ((u == s || v== t) ? 0 : (cost[e] + pot[u] - pot[v]))
...
{ 
  ...
  { 
    ...
    if (excess[v] > 0 && v != t && v != s) U.insert(v,d);
...
void handle_gap(const graph& G, node v, node s, node t, const cap_array& cap,
                                                const flow_array& flow,
                                                dist_array& dist, 
                                                succ_array& succ, 
...
template<class cap_array, class flow_array, class excess_array,
                                            ...
                                            class cost_array, 
                                            class pot_array>
void phase1(const graph& G, node s, node t, const cap_array& cap,
                                            flow_array& flow,
                                            excess_array& excess,
                                            dist_array& dist,
                                            succ_array& succ,
...
{ 
  ...
  compute_dist1(G,s,t,cap,flow,excess,succ,dist,U,cost,pot,count);
  ...
  while ((v = U.find_max()) != NULL_NODE) 
  {
    ...
    if (--count[dv] == 0) // gap
    { handle_gap(G,v,s,t,cap,flow,dist,succ,cost,pot,count);
      goto NEXT_NODE;
     }
...
NT run(const graph& G, node s, node t, const cap_array& cap, 
                                             flow_array& flow, 
                                             excess_array& excess,
                                             dist_array& dist,
                                             succ_array& succ,
...
{ 
  ...
  edge e;
/*
  forall_out_edges(e,s) excess[s] -= flow[e];
  forall_in_edges(e,t) excess[t] += flow[e];
*/

  forall_out_edges(e,s) 
  { node u = target(G,e,s);
    if (RCOST(e,s,u) != 0) continue;
    NT c = cap[e]-flow[e];
    if (c < delta) continue;
    flow[e] = cap[e];
    excess[s] -= c;
  ...
  phase1(G,s,t,cap,flow,excess,dist,succ,cost,pot);
  phase1(G,t,s,cap,flow,excess,dist,succ,cost, pot);
