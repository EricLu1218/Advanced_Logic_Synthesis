...
#define LEDA_ROOT_INCL_ID 520228
...
max_flow() : h(5.0),fval(0),cut_bound(0)
{ reset_counters(); }
...
template<class cap_array, class flow_array>
NT run(const graph& G, node s, node t, const cap_array& cap, flow_array& flow, bool init = true)
{ 
  float T = used_time();
  int n = G.number_of_nodes();
