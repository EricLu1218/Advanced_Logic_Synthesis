...
#define LEDA_ROOT_INCL_ID 520213
...
private:
  ...
  typedef member_array<int,  nd_array, node_data> curr_array;
...
// dynamic in_edge lists 

  edge* first_in_E;
  edge* next_in_E;

  edge FirstInEdge(const graph& G, node v) const 
  { return first_in_E[G.index(v)]; }

  edge NextInEdge(const graph& G, edge e)  const 
  { return next_in_E[G.index(e)]; }

  void InitInEdges(const graph& G)
  { int n = G.number_of_nodes();
    int m = G.number_of_edges();
    first_in_E = new edge[n];
    next_in_E  = new edge[m];
    node v;
    forall_nodes(v,G) first_in_E[G.index(v)] = v->first_in_edge();
    edge e;
    forall_edges(e,G) next_in_E[G.index(e)] = e->next_in_edge();
    G.clear_in_edges();
   }

  void DeleteInEdges(const graph& G)
  { if (first_in_E)
    { G.rebuild_in_edges();
      delete[] first_in_E;
      delete[] next_in_E;
...
max_flow() : h(5.0), run_phase0(true), fval(0), cut_bound(0), first_in_E(0),
                                                              next_in_E(0)
...
{ 
  ...
  { edge e;
    ...
    { if (out_cap(e) == 0) continue;
      ...
      if (dist[v] > dist[w]+1) 
      { err_count++;
        cout << string("out_edge: dv = %d  dw = %d c = %d f = %d",
                        dist[v],dist[w],cap[e],flow[e]) << endl;
        return false;
       }
     }
    forall_in_edges(e,v)
    { if (in_cap(e) == 0) continue;
      ...
      if (dist[v] > dist[w]+1)
      { err_count++;
        cout << string("in_edge: dv = %d  dw = %d",dist[v],dist[w]) << endl;
        return false;
       }
     }
   }
...
//double  cut = 0;
  ...
  //cut += excess[v];
    ...
    { edge e;
      if (first_in_E)
        for(e=FirstInEdge(G,v); e; e = NextInEdge(G,e))
        { edge_insp++;
          node u = source(G,e,v); 
          int du = dist[u];
...
*/
 ...
 //if ((queue_last-queue+1) < n-1) // not all nodes of V reached
 if ((queue_last-queue+1) < n-1  && edge_insp >= max_edge_insp)
 { d--;
   node v;
   forall_nodes(v,G)
...
//double  cut = 0;
  ...
  { 
    ...
    current[v] = (v->first_out_edge() - f_edge);
                                                   ...
                                                   curr_array& current)
{
  ...
  current[t] = (t->first_out_edge() - f_edge);
...
*/
  ...
  int sz = 0;
  for(node* p = queue; p < queue_cur; p++)  V[sz++] = *p;
  V[sz] = 0;
    ...
    while (ev > 0 && dv < n)
    {
      ...
      { insp_count++;
        ...
        { push_count++;
          ...
          if (!G.test_in_edge(e)) G.push_in_edge(w,e);
          if (ev <= x) 
          { change_flow(e,ev); 
            excess[w] = ev+ew;
            ev = 0; 
      ...
      { f_push_count++;
        ...
        if (rmin > 0)
          { change_flow(emin,ev)
            if (!G.test_in_edge(emin)) G.push_in_edge(w,emin);
           }
        else
            change_flow(emin,-ev)
        NT ew = excess[w];
...
template<class cap_array, class flow_array>
bool check_current_edge(const graph& G, node v, int cur_i, 
                                                const cap_array& cap,
                                                const flow_array& flow,
...
{ 
  edge f_edge = G.first_edge() - 1;
  edge cur_e = (cur_i > 0) ? (f_edge + cur_i) : (f_edge - cur_i); 

  int dv = dist[v];

  edge e;
  forall_out_edges(e,v)
  { if (e == cur_e) return true;
    NT x = out_cap(e);
    if (x==0) continue;
    node w = target(G,e,v);
    if (dist[w] == dv-1) return false;
   }
...
{ 
  ...
  while ((v = U.del_max()) != t) 
  {
    ...
    //assert(check_current_edge(G,v,cur_i,cap,flow,dist));
    ...
    for(;;)
    {
      ...
      { V_sz = compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,current);
        cur_i = current[v];
      ...
      { NT x = out_cap(e);
        ...
        node w = target(G,e,v);
        int dw = dist[w];
        if (v != w && dw < dmin1) { dmin1 = dw; emin1 = e; rmin1 = x; }
       }
      ...
      cur_i = (rmin > 0) ? emin-f_edge : f_edge-emin;
...
{ 
  ...
  curr_array   current(n_data,  &node_data::curr);
  ...
  if (m > 10*n) InitInEdges(G);

  cout << endl;
  cout << string("init:   %.2f sec",used_time()-T) << endl;
  ...
  { node u = target(G,e,s);
    ...
    excess[s] -= c;
    excess[u] += c;
    if (!G.test_in_edge(e)) G.push_in_edge(u,e);
  }


  compute_dist0(G,V,s,t,cap,flow,excess,dist,U,count,current);

  if (run_phase0) 
  { phase0(G,V,s,t,cap,flow,excess,dist,U);
    compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,current);
   }
  cputime[0] = used_time(T);

  //print_edge_statistics(G,cap,flow);


  phase1(G,V,s,t,cap,flow,excess,dist,count,U,current);
