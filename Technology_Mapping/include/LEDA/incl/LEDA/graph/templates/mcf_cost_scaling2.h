...
#define LEDA_ROOT_INCL_ID 520239
...
{ // precondition: (flow, pot) must be 1-optimal
  ...
  node_array<rcost_t,graph_t> dist(G);
  node_pq<rcost_t,graph_t> PQ(G);
...
template<class cap_array, class flow_array, class cost_array, class ex_queue>
void topsort_admissible(const graph_t& G, const cap_array& cap,
                                          const cost_array& cost,
                                          const flow_array& flow,
                                          ...
                                          ex_queue& active)
{
  ...
  bool acyclic_admissible = (q == Q);

  delete[] Q;

  assert(acyclic_admissible);
}
...
*/
  ...
  node v;
  forall_nodes(v,G) 
  { pred[v] = 0; // tree predecessor (0: unvisited, v: completed)
  ...
  { 
    ...
    { 
      ...
      while (e)
      { node w = G.opposite(e,v);
        if (pred[w] != w  && // skip edges to completed nodes (forward/cross)
      ...
      { *--q = v;         // add to front of topsort list
        ...
        v = u;
/* 
        // not necessary since current edge points to a completed node
        if (v != sentinel)
        { incident_edge e = current[v];
          current[v] = G.next_incident_edge(e,v);
         }
*/
        continue;
       }
      ...
      { // w visited but not completed: backward edge
        ...
        for(node u = pred[v]; u != pw; u = pred[u])
        ...
        { incident_edge e = current[u];
          push_flow(e,min_rc);
          if (G.hidden_in_edge(e))
          { node w = G.opposite(e,u);
                                                     ...
                                                     node_pq& PQ)
{
  ...
  while (topsort_dfs(G,cap,cost,flow,pot,pred,current,Q))
  {
...
*/
    ...
    if (PQ.max_level() == 0) // eps-optimal
    { result = true;
      break;
     }

    if (PQ.max_level() > max_level) max_level = PQ.max_level();

    for(int level=PQ.max_level(); level >= 0; level--)
    { node v;
      forall_nodes_in_bucket(v,PQ,level)
      { assert(level == -dist[v]);
        ...
        { if (out_cap(e) == 0) continue;
          ...
          if (d == dv)  
            PQ.insert(w,v); // insert w on current level after v
          else
            PQ.push(w,-d);  // insert w at front of level -d
                                     ...
                                     bool rebuild_active)
{
  ...
  int min_dist = max_dist - inf;

  excess_t sum_excess = 0;

  PQ.clear(1); // bucket[0] contains all deficit nodes

  node v;
  forall_nodes_in_bucket(v,PQ,0)
  ...
  while (++level <= PQ.max_level() && sum_excess != 0)
  { node u;
    forall_nodes_in_bucket(u,PQ,level)
    { du = level+min_dist;
      assert(du == dist[u]);

      excess_t eu = excess[u];
      if (eu > 0)
      { sum_excess += eu;
      ...
      { if (in_cap(e) == 0) continue;
        ...
        node v = G.opposite(e,u);
        int dv = dist[v];
        //if (dv > max_dist) dv = max_dist;
        if (du >= dv) continue;

        rcost_t rc = -n_plus_1*cost[e]-pot[v] + pot_u;

        int c = du + int(1+rc/eps); 
        assert(c >= du);
        if (c >= dv) continue;
    
        if (dv < max_dist) PQ.remove(v,dv-min_dist);
    
        if (c == du)  
           PQ.insert(v,u);       // insert v on current level after u
        else
           PQ.push(v,c-min_dist);// insert v as first node on level c
...
*/
  ...
  // update potential of all permanent labeled nodes (level <= du)

  du -= min_dist;

  int p_labeled = 0;
  for(int i = du; i >= 0; i--)
  { rcost_t dpot = (i-du)*eps;
    node v;
    forall_nodes_in_bucket(v,PQ,i)
    { pot[v] += dpot;
      if (rebuild_active && excess[v] > 0) active.append(v);
      p_labeled++;
     }
   }
  ...
  if (write_log)
  { float r = (100.0*p_labeled)/n;
    cout << string("update: eps = %8.2f  r = %5.1f %%  max = %3d   %5.2f sec",
                            eps, r, PQ.max_level(), used_time(t1)) << endl;
   }

  update_limit = int(1.5*p_labeled);
  if (update_limit < 16) update_limit = 16;
...
int filter_in_edges(const graph_t& G, cap_array& cap, 
                                      flow_array& flow,
                                      excess_array& excess)
{ G.hide_all_in_edges();
  int count = 0;
                                                ...
                                                rcost_t eps)
{
  ...
  { double dv = 0;
    ...
    { node w = G.opposite(e,v);
      ...
      if (ew > excess_max || ew < -excess_max) 
...
*/
  ...
  while (eps > 1)
  {
    ...
    { establish_eps_optimality(G,cap,cost,pot,flow,excess,eps);
      //eliminate cycles in the admissible network
      //node* Q = new node[n];
      //topsort_dfs(G,cap,cost,flow,pot,pred,current,Q);
      //delete[] Q;
     }


    // reset current edges, intialize dist array and bucket queue
    // (bucket(0) = list of all deficit nodes)
    ...
    if (!global_update(G,cap,cost,flow,excess,pot,dist,current,pred,
                                                       PQ,active,eps,true))
    { if (refine_count == 1)
      { feasible = false;
