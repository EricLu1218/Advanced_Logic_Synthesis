...
// $Revision: 1.15 $  $Date: 2007/02/28 07:44:11 $

#ifndef LEDA_ARRAY_H
#define LEDA_ARRAY_H

#if !defined(LEDA_ROOT_INCL_ID)
#define LEDA_ROOT_INCL_ID 520523
...
{ if (low > high+1) 
    error_handler(1,string("array: bad array bounds [%d..%d]",low,high));
  l = low;
  h = high;
  sz = high-low+1;
  vec = (sz > 0) ? new E[sz] : 0;
...
if (i<l || i>h) 
    error_handler(99,string("array: index %d out of bounds",i));
#endif
return vec[i-l];
}
...
{ if (low < l || low > h || high < l || high > h || low > high)
    error_handler(99,"array::permute illegal range");
  E* last = vec+(high-l);
  for(E* p = vec+(low-l); p < last; p++) 
    leda_swap(*p,*(p+rand_int(1,last-p)));
}
...
void sort(const leda_cmp_base<E>& cmp) { QUICKSORT_CMP(vec,vec+sz-1,cmp); }

void sort(int (*cmp)(const E&, const E&)) { QUICKSORT_CMP(vec,vec+sz-1,cmp); }
...
void sort() { QUICKSORT_C(vec,vec+sz-1); }
...
void sort1() { QUICKSORT_C1(vec,vec+sz-1); }


void sort(const leda_cmp_base<E>& cmp, int low, int high)
{ QUICKSORT_CMP(vec+low-l,vec+high-l,cmp); }
...
{ QUICKSORT_C(vec+low-l,vec+high-l); }
...
{ E* p = BINARY_SEARCH_C(vec,vec+sz-1,x);
  return (compare(*p,x)==0) ? l+(p-vec) : l-1;
}
