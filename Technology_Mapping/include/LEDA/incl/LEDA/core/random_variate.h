...
#define LEDA_ROOT_INCL_ID 520452
...
/*{\Mcreate creates an instance |\Mvar| of type |\Mname|. }*/
  ...
  { 
    ...
    if ( n == 0 ) 
           error_handler(1,"random_variate: no nonzero weight");
...
/*{\Mcreate creates an instance |\Mvar| of type |\Mname|. }*/
  { 
    ...
    { u[n + i] = w[l + i];
      if ( u[n + i] < 0 )
        error_handler(1,"dynamic variate: negative weight");
    }
    for (i = n - 1; i > 0; i--)
      u[i] = u[2*i] + u[2*i + 1];

    if (u[1] == 0 ) 
        error_handler(1,"dynamic variate: no non-zero weight");
 }
...
\precond $i \in \range{l}{h}$.}*/
  { 
    ...
    u[1] += delta;
    if ( u[1] == 0 ) 
      error_handler(1,"dynamic variate: no positive weight");
    return ui;
 }
