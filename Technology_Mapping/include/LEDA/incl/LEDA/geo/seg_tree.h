...
// $Revision: 1.3 $  $Date: 2007/02/25 00:48:32 $
...
#include <LEDA/core/impl/bb1_tree.h>
...
{ if (!it) error_handler(1,"seg_nd_tr_item is nil");
...
class __exportC Segment_Tree  : public bb1_tree {
  ...
  { error_handler(1,"Segment_Tree::new_y_h_segment"); return 0; }
  ...
  GenPtr&        key(bb1_item it)       
                 { if (!it) error_handler(1,"bb1_item in segment_tree is nil");
	               return it->key; }
...
public:
  ...
  { error_handler(1,"Segment_Tree::cmp"); return 0; }
